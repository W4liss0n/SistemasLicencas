...
- Mutacoes de licensing (`POST /licenses*`, `PATCH /licenses/:licenseKey`, `renew`, `block`, `unblock`, `cancel`): `Idempotency-Key`.
- Mutacoes de catalogo de planos (`POST /plans`, `PATCH /plans/:planId`): `Idempotency-Key`.
...
- Edicao de licenca atualiza `subscription.end_at`, `subscription.auto_renew` e `license.max_offline_hours`.
- Renovacao atualiza `subscription.end_at` e reativa assinatura quando aplicavel.
- Edicao de plano atualiza limites, descricao e vinculos com programas.
